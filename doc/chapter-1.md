...
Не известно сколько ещё __современных__ Java-проектов используют `SOAP`. 
...
*В этом документе мы постараемся взглянуть на SOAP с другой стороны. Теория будет представлена очень тезисно, её хватает в сети.*

**Запомните. Не стремитесь сразу же читать WSDL файлы. Это только введёт в заблуждение. Они создаются роботами для роботов. 
Начните с самого минимального примера, и поняв, как он работает, можно будет разобраться и с более тяжёлыми задачами.**
...
В самых первых примерах мы в основном будем использовать реализацию по умолчанию. 
В контейнерах `Jboss / WildFly`, кстати, по-умолчанию используется реализация `Apache CXF`, к неё мы перейдём позднее. 
