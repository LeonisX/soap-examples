...
Идея пространства имён сильно напоминает идею, по которой в Java существуют пакеты. 
...
`SOAP` (простой протокол доступа к объектам) это протолок обмена XML-сообщениями по сети, обычно с использованием HTTP-протокола. 
...
связанную с безопасностью и транзакциями, а то время как тело содержит XML-сообщение.
...
и сообщения (типы входных и выходных параметров для методов), которые необходимы для вызова сервиса. 
...
Так что обычно RPC используется в сочетании с правилами кодирования SOAP, обычно это записывается как `RPC/Encoded`. 
...
Он взовращает строку как "`rstring`".
...
**Разница только в том, как сообщения будут описаны в `WSDL-файле`. Поскольку всё генерируется автоматически, то программиста разницы особой нет.**
